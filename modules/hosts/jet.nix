...
}:
{
  ...
  flake.homeConfigurations.full = inputs.home-manager.lib.homeManagerConfiguration {
    pkgs = withSystem "x86_64-linux" ({ pkgs, ... }: pkgs);
    modules = [
      self.modules.homeManager.jet
    ];
  };

  flake.nixosConfigurations.jet = inputs.nixpkgs.lib.nixosSystem {
    pkgs = withSystem "x86_64-linux" ({ pkgs, ... }: pkgs);
    modules = [
      ./_hardware/jet.nix
